// --- SISTEMA TÉCNICO AVANÇADO DE ANÁLISE QUÍMICA E CONTROLE DE VOZ ---

let textoUltimoAlerta = ""; // Guarda o texto para a função "Repetir"

function falarTexto(mensagem) {
    window.speechSynthesis.cancel(); // Para qualquer fala ativa imediatamente
    
    const falar = new SpeechSynthesisUtterance(mensagem);
    
    // --- MOTOR DE VOZ ULTRA NATURAL ---
    // Procura por vozes nativas em PT-BR de melhor qualidade instaladas no navegador
    ...
    falar.lang = 'pt-BR';
    falar.rate = 0.9;  // VELOCIDADE CORRIGIDA: Fala pausada e muito mais natural
    falar.pitch = 1.0; // Tom de voz equilibrado
    
    window.speechSynthesis.speak(falar);
}

// Garante o carregamento correto das vozes em navegadores como o Chrome
...
function calcularMistura() {
    ...
    if (pA === pB) {
        ...
        atualizarInterface("seguro", "✓", tit, desc, ["Luvas de Proteção"]);
    ...
    atualizarInterface(tipo, icone, titulo, descricao, epis);
...
function acionarNarracao(titulo, descricao) {
    // Insere uma pequena pausa natural de pontuação estrutural na fala
    textoUltimoAlerta = `${titulo}. ... Atenção para a explicação técnica: ... ${descricao}`;
    falarTexto(textoUltimoAlerta);
}

function repetirAudio() {
    if (textoUltimoAlerta) {
        falarTexto(textoUltimoAlerta);
    }
}
...
function adicionarAoHistorico(prod1, prod2, tipo) {
    ...
    // Mantém as consultas mais recentes no topo
    lista.insertBefore(item, lista.firstChild);

    // Limita o histórico visual aos últimos 5 itens para não quebrar o layout
